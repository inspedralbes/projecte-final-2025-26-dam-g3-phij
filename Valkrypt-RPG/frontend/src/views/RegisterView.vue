    ...
    <div class="auth-box">
      ...
      <form @submit.prevent="handleRegister" class="auth-form">
        <div class="input-group">
          ...
          <input v-model="username" type="text" placeholder="Tu nombre en el reino..." required />
        </div>

        <div class="input-group">
          <label>CORREO ELECTRÓNICO</label>
          <input v-model="email" type="email" placeholder="valkrypt@ejemplo.com" required />
        </div>
...
const handleRegister = async () => {
  try {
    const response = await fetch('http://135.181.82.185:3000/api/register', {
      method: 'POST',
...
.auth-container {
  ...
  background-image: 
    radial-gradient(circle at center, #1a0808 0%, #050505 100%),
