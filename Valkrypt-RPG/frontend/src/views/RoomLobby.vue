  ...
  <div class="room-lobby">
    <div class="lobby-container">
      <header class="lobby-header">
        <h1>⚔️ VALKRYPT - Sala Cooperativa</h1>
        <p>Crea o unete a una sala para coordinar acciones y jugar en grupo.</p>
      </header>
      ...
      <div class="lobby-content">
        ...
        <section class="active-rooms-section">
          <h2>Salas disponibles ({{ availableRooms.length }})</h2>
          <div v-if="loadingRooms" class="loading">Cargando salas...</div>
          ...
          <div v-else class="rooms-grid">
            ...
            >
              ...
              <div class="room-info">
                <p><i class="fas fa-users"></i> {{ room.players.length }}/{{ room.maxPlayers }} jugadores</p>
                <p><i class="fas fa-user-crown"></i> Host: {{ room.hostName }}</p>
                <p><i class="fas fa-hourglass-half"></i> Estado: {{ room.status === 'in-progress' ? 'en progreso' : 'esperando' }}</p>
              </div>
              ...
              <button
                @click="joinRoom(room)"
                class="btn-join"
                :disabled="loading || (room.status === 'in-progress' && !isUserInRoom(room)) || (room.players.length >= room.maxPlayers && !isUserInRoom(room))"
              >
...
const joinRoom = async (room) => {
  ...
  try {
    ...
    if (response.data.success) {
      success.value = 'Te has unido. Entrando a la sala...';
